...
---new experiment---
- **Date/time:** 2026-04-27T08:43:55.854003
- **Branch:** autoresearch/clean_transcript_20260426_000
- **Commit:** NO_COMMIT
- **Hypothesis:** corr_02 scores 7.5 in the current best (102.5). The consistent counterexamples show the model: (1) stating character inferences as confirmed world-facts ('recognized as Danielle' when transcript shows 'we think that's Danielle'), (2) dropping conditional caveats from theories ('theorized X' but omitting 'assuming he didn't orchestrate it'). The only previous change that improved the score was adding sub-bullets to items 5 and 6 with bold headers and concrete examples. The corr_02 failures involve character INFERENCES and THEORIES — which belong conceptually under item 5 (Thoughts, Feelings, Intentions, and Suspicions). Adding a sub-bullet there with bold header and concrete right/wrong example targets corr_02 at the right location. The example specifically shows the character-inference-to-confirmed-fact pattern and the caveat-preservation requirement. Unlike previous corr_02 attempts (which modified item 3, Critical Requirements, or added sections), this targets item 5 where prior additions succeeded.
- **Change:** Added a third sub-bullet to item 5 (Thoughts, Feelings, Intentions, and Suspicions) with bold header 'Write inferences as inferences.' The sub-bullet instructs: when a character concludes/suspects/believes something not independently confirmed by transcript, use belief language (believed, suspected, concluded, theorized) rather than stating as confirmed fact. Provides concrete example: 'We think that's Danielle' should produce 'The group believed the figure was Danielle,' not 'The group recognized Danielle.' Also instructs to preserve conditional caveats when theories include them.
- **Result:** discard
- **Result summary:**
  - Total score: 97.125
  - Low-scoring criteria (6):
    ...
    - comp_02 6.5 - session_log includes separate entries or clearly separable sentence-level beats for the distinct in-game actions, events, observations, discoveries, revelations, spoken statements, and situation changes that occur during live play, rather than collapsing them into only major plot summaries.
    - comp_03 8.1 - The logs preserve the player characters’ explicit decisions, plans, intentions, stated thoughts, suspicions, and stated changes in understanding when those are present in the transcript.
    - comp_04 6.9 - The logs account for the fictional consequences of attempts and conflicts, including successes, failures, partial successes, costs, harm, recovery, delays, changed risks, and newly available options when present in the transcript.
    - corr_01 9.4 - Every entry in the logs is directly supported by the raw transcript. Concise paraphrase is allowed, but the logs must not introduce hallucinated events, fabricated dialogue, new causal claims, unsupported interiority, or outcomes not explicitly supported by the transcript.
    - corr_02 7.5 - The logs preserve uncertainty and attribution from the transcript, using cautious wording when facts, speakers, character identities, perceptions, or outcomes are unclear rather than over-resolving them.

---new experiment---
- **Date/time:** 2026-04-27T08:56:56.741180
- **Branch:** autoresearch/clean_transcript_20260426_000
- **Commit:** NO_COMMIT
- **Hypothesis:** corr_02 scores 7.5 in the best run (102.5). The counterexamples consistently show: character inferences stated as confirmed facts (group 'recognized' Danielle when they only 'believed' it was her), conditional caveats dropped from theories. The existing uncertainty rule says 'but only when the transcript itself is uncertain' — this is too restrictive. The model interprets 'uncertain transcript' as explicitly hedged transcript language, but misses cases where the CHARACTERS are drawing inferences that the transcript reports without explicit uncertainty markers. Replacing 'but only when the transcript itself is uncertain' with 'when the transcript expresses something as an inference, belief, or possibility' makes the trigger condition more precise: it's about the NATURE of what's being expressed (inference vs confirmed fact), not just whether the transcript uses uncertain language. This is a same-sentence replacement, not an addition, targeting the precise wording change that should fix corr_02.
- **Change:** Replaced the uncertainty sentence in the Live Session Log section. Old: 'If the transcript is uncertain, preserve that uncertainty with wording such as appeared to, seemed to, or the group believed, but only when the transcript itself is uncertain.' New: 'Preserve the transcript's level of certainty: use wording such as appeared to, seemed to, or believed when the transcript expresses something as an inference, belief, or possibility — and use direct narration when the transcript states it as confirmed fact or direct action.' This clarifies the trigger: use hedged wording for inferences/beliefs/possibilities, not just when the transcript contains explicit uncertainty markers.
- **Result:** discard
- **Result summary:**
  - Total score: 96.625
  - Low-scoring criteria (8):
    - comp_01 7.8 - If the transcript contains a recap of past in-game events, recap_log captures the explicit recap as separate concrete beats rather than collapsing multiple events, discoveries, decisions, clues, relationships, unresolved problems, ongoing risks, or prior developments into vague summary statements.
    - comp_02 8.8 - session_log includes separate entries or clearly separable sentence-level beats for the distinct in-game actions, events, observations, discoveries, revelations, spoken statements, and situation changes that occur during live play, rather than collapsing them into only major plot summaries.
    - comp_03 5.9 - The logs preserve the player characters’ explicit decisions, plans, intentions, stated thoughts, suspicions, and stated changes in understanding when those are present in the transcript.
    - corr_01 8.8 - Every entry in the logs is directly supported by the raw transcript. Concise paraphrase is allowed, but the logs must not introduce hallucinated events, fabricated dialogue, new causal claims, unsupported interiority, or outcomes not explicitly supported by the transcript.
    - corr_02 8.8 - The logs preserve uncertainty and attribution from the transcript, using cautious wording when facts, speakers, character identities, perceptions, or outcomes are unclear rather than over-resolving them.
...
---new experiment---
- **Date/time:** 2026-04-27T09:24:44.460678
- **Branch:** autoresearch/clean_transcript_20260426_000
- **Commit:** NO_COMMIT
- **Hypothesis:** corr_02 scores 7.5 in the best run (102.5). The counterexamples show the model writing character inferences as definitive facts. The 'Translate Mechanics to Fiction' section provides the main writing style examples for entries — both of which show confident, definitive past-tense statements. This may be anchoring the model toward definitive language for all entries, including ones that should be hedged. Adding 'preserving the level of certainty in the outcome: use direct narration for clear outcomes, hedged wording for uncertain ones' to the principle line in Translate Mechanics to Fiction gives the model explicit guidance that entry style should match the certainty level of the source, without adding a new section or changing the examples. This is a sentence-level edit within an existing section.
- **Change:** Modified the second bullet in 'Translate Mechanics to Fiction' from 'Record what the character attempted and what happened in the fiction.' to 'Record what the character attempted and what happened in the fiction, preserving the level of certainty in the outcome: use direct narration for clear outcomes, hedged wording for uncertain ones.' This adds a certainty-preservation caveat to the core entry-writing principle without changing the existing examples.
- **Result:** discard
- **Result summary:**
  - Total score: 96.125
  - Low-scoring criteria (7):
...
---new experiment---
- **Date/time:** 2026-04-27T11:09:01.803064
- **Branch:** autoresearch/clean_transcript_20260427_000
- **Commit:** NO_COMMIT
- **Hypothesis:** comp_02 scores 7.1 in the current best (101.75). The rubric penalizes collapsing distinct session events into combined entries. The only successful change so far was adding 'Each distinct recap fact must be its own entry. Do not combine two events, discoveries, decisions, or clues into a single entry' to the Recaps section — this improved comp_01 from 8.8 to 9.4. The exact same structural principle applied to the Live Session Log section should improve comp_02 by the same mechanism. Adding 'Each distinct in-game beat must be its own entry. Do not combine two actions, events, observations, revelations, or consequences into a single entry — if the transcript states three separate things happened, that is three entries, not one' mirrors the successful Recaps instruction and gives the model an explicit one-beat-per-entry rule for session_log.
- **Change:** Added one bullet to the Live Session Log section: 'Each distinct in-game beat must be its own entry. Do not combine two actions, events, observations, revelations, or consequences into a single entry — if the transcript states three separate things happened, that is three entries, not one.' This mirrors the bullet added to the Recaps section that successfully improved comp_01, applying the same structural principle to session_log.
- **Result:** discard
- **Result summary:**
  - Total score: 101.75
  - Low-scoring criteria (5):
...
---new experiment---
- **Date/time:** 2026-04-27T11:52:45.667513
- **Branch:** autoresearch/clean_transcript_20260427_000
- **Commit:** NO_COMMIT
- **Hypothesis:** comp_02 scores 7.1 in the current best (101.75). The model consistently omits specific investigation findings (Danielle's friends and the others reference, old orphanage bought by Maryland Prison, Roth helping bypass background checks, Sugar Cookies death). The 'Repeated Non-Information' rule says 'Collapse pure repetition that adds no new fictional information.' The model may interpret these investigation findings as 'repetition' of a broader theme already captured (Philip Roth connected to Danielle, private prison network) and collapse them. Removing this rule entirely eliminates a potential justification for omitting these details. The risk is the model will include more actual repetition, but given that comp_02 is currently the main drag at 7.1, the tradeoff may be net positive. This removes the only compression rule that could be used to justify omitting valid distinct beats.
- **Change:** Removed the 'Repeated Non-Information' bullet (item 3) from the What to Remove section. The removed text was: 'Collapse pure repetition that adds no new fictional information. If repeated questioning produces a new in-game fact, capture the new fact.' This eliminates the only rule that could justify omitting specific investigation findings as alleged repetition.
- **Result:** discard
- **Result summary:**
  - Total score: 93.75
  - Low-scoring criteria (7):
